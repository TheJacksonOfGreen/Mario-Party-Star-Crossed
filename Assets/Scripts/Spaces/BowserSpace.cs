    ...
    public override IEnumerator land(Player p) {
        ...
        switch (p.state.charName()) {
            ...
            case "Toad":
                ...
                yield return new WaitUntil(() => ui.WaitForDialogueAnswer());
                ui.Dialogue("Bowser", "Oh, wait, I forgot...you're just a tiny little Toad! All you can do is PANIC! Gwahahaha!", false);
                yield return new WaitUntil(() => ui.WaitForDialogueAnswer());
                break;
            case "Rosalina":
            ...
            case "Hammer Bro":
                ...
                break;
            case "Dry Bones":
                ui.Dialogue("Bowser", "Hey, Dry Bones, why are you even at this party? It's not like you have any-BODY to go with! Gwa ha ha!", false);
                yield return new WaitUntil(() => ui.WaitForDialogueAnswer());
        ...
        if (p.state.getItems().Count >= 2) {
            normalEvents.Add("Lose all of Your Items");
        }
        if (st[3] != p.state && p.state.getCoins() >= 16) {
        ...
        ui.Spinner("Bowser Time!", bowserOptions);
        yield return new WaitUntil(() => ui.WaitForDialogueAnswer());
        switch (ui.MostRecentDialogueAnswer()) {
            ...
            case "Bowser Revolution":
                ...
                if (totalCoins % 4 != 0) {
                    yield return new WaitUntil(() => ui.WaitForDialogueAnswer());
                    ui.Dialogue("Bowser", "And, as a fee for my services, I'll just take the extra " + (totalCoins % 4) + "!", false);
                }
                yield return new WaitUntil(() => ui.WaitForDialogueAnswer());
                ui.Dialogue("Bowser", "Now, you're all equal! See, isn't that just so much better?", false);
                yield return new WaitUntil(() => ui.WaitForDialogueAnswer());
                ui.Dialogue("Bowser", "Hope to see you on another Bowser Space real soon!", true);
                yield return new WaitUntil(() => ui.WaitForDialogueAnswer());
                break;
            case "Bowser Shuffle":
                ...
                yield return new WaitUntil(() => ui.WaitForDialogueAnswer());
                ui.Dialogue("Bowser", "Sorry, I wasn't listening to what you said. Anyway, here we go!", false);
                yield return new WaitUntil(() => ui.WaitForDialogueAnswer());
